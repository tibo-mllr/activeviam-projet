...
export async function convertV1toJson(input: string) {
  const v1Collection = await parseMultiV1(input);
...
function convertToV2(v1Structure: V1Structure): {
  errors: Error[];
  result: unknown;
} {
...
function mapAggregateRetrieval(
  ...
  return {
    ...
    ...(retrieval.properties["Partial provider"] !== undefined
    ...
    ...(retrieval.properties["Measures provider"] !== undefined
...
async function parseMultiV1(input: string): Promise<V1Structure[]> {
  ...
  for (let i = 1; i < sectionHeaders.length; ++i) {
    parts.push(
      await parseV1(lines, sectionHeaders[i - 1], sectionHeaders[i]).then(
        (part) => {
...
function parseProps(state: V1Structure, line: string) {
  ...
  if (newProp) {
    state.info[newProp] = newProp !== "rangeSharing" ? value : parseInt(value);
  } else {
...
type V1Info = {
  ...
  [key in V1InfoKeyType]?: string | number;
};
...
const VirtualRetrievalKind = "VirtualRetrieval" as const;
const AggregateRetrievalKind = "AggregateRetrieval" as const;
const ExternalRetrievalKind = "ExternalRetrieval" as const;
const CondensedRetrievalKind = "CondensedRetrieval" as const;
...
type AggregateRetrieval = ARetrieval & {
...
type ExternalRetrieval = ARetrieval & {
...
class Dictionary<T> extends Map<T, number> {
  index(key: T): number {
    if (!this.has(key)) {
